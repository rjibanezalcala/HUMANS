...
# ------------------------------ App functions --------------------------------
def parse_ini(filename="bin/settings.ini", section="postgresql", eval_datatype=False):
...
def validate(theoretical, real, inplace=False):
    ...
    for mkey, sdict in validated.items():
        for key, target in sdict.items():
            if key != "topic_id":
                validated_list = [
                    x for x in target if x in real[key.replace("-", "_").lower()]
                ]
...
def import_demdata(
    subjectid, credentials, exclude_keys=["num_stories", "next_story_index"]
):
...
def replace_demdata(user, target_entries, make_backup=True):
    ...
    if make_backup:
        if not os.path.exists(data_dir + "/demographic_info_old.txt"):
            copy2(
                ...
                data_dir + "/demographic_info_old.txt",
            )
    ...
    with open(
        data_dir + "/demographic_info.txt",
        "w",
        errors=app_settings.get("encoder_error", "ignore"),
    ) as f:
        f.writelines(data)
    ...
    for key in dictionary:
        for x in dictionary[key][task_search]:
            if int(x) == int(story_search):
                result.update(
                    {
                        "topic_id": dictionary[key]["topic_id"],
                        "topic": key,
...
def get_new_id(reference_from="database"):
    ...
    if reference_from == "database":
        ...
        path += "/data/"
        unique_ids = set(
            [int(x) for x in re.findall("\d+", " ".join(os.listdir(path)))]
        )
...
def get_story_order(
    subjectidnumber, ignore_legacy_story_data=0, validate_only=False, to_validate=[]
):
    ...
    if len([s for s in story_order if str(s).isnumeric()]) == len(story_order):
        if ignore_legacy_story_data:
            result.update({"STO_CH": 1, "story_order": []})
            print(
            ...
            result.update(
                {
                    "STO_CH": 0,
                    "story_order": [
                        f"/approach_avoid/story_{str(s)}" for s in story_order
                    ],
                }
            )
            print(
        ...
        result.update({"STO_CH": 0, "story_order": story_order})
        return result


def get_starting_story_indx(
    subjectidnumber, stories_in_session, reference_from="database", db_method="count"
):
        ...
        try:
            ...
            try:
                num_stories_completed = int(
                    get_demographic_info(subjectidnumber)["next_story_index"]
                )
            except Exception as error:
                print(
...
def choose_questions(sesh):
    ...
    print(
        f"\nChoosing questions for current story: {story_num_overall}."
    )  # Should help with debugging
    path = os.path.abspath(f"stories/task_types{story_num_overall}/questions.txt")
    txt = open(
        path,
        encoding=app_settings.get("txt_encoding", "utf-8"),
        errors=app_settings.get("encoder_error", "ignore"),
    ).read()
    lines = txt.split("\n")
    ...
    for l in lines:
        ...
        linelist[1] = re.findall("\(.*?\)", l)[
            -1
        ]  # Find everything in parenthesis and take the last element.
        question = linelist[0]
        if task_type == "social":
            if (
                app_settings["randomise_relation_levels"]
                and story_num in app_settings["relation_level_stories"]
            ):
                print(f"\nRandomising relationship levels in question {linelist[1]}")
                question, _ = replace_all(
                    question,
                    app_settings["relation_levels"],
                    replace_with=relationship_level,
                )
        RC = re.findall(
            "\d+", linelist[1]
        )  # Take only the numeric values in this part of the string.
    ...
    if task_type == "benefit_benefit":
        ...
        relevant_keys = list(
            itertools.product(
                rewards, costs, repeat=(2 if task_type == "multi_choice" else 1)
            )
        )
    ...
    relevant_qs = {
        key: quest_dict[key] for key in relevant_keys if key in list(quest_dict.keys())
    }
...
def write_trial_to_db(
    ...
    path_start = (
        trial_start[-1] if isinstance(trial_start, tuple) and record_path else None
    )
    ...
    dem_dict.update(
        {
            "tasktypedone": session["story_num_overall"],
            ...
            else ";".join([trial_start, path_start]),
            "trial_end": trial_end
            if path_end is None
            else ";".join([trial_end, path_end]),
            "trial_elapsed": trial_elapsed,
            "eye_tracker_data": {
                "gaze_data": eyetracker.gaze
                if eye_settings["use_eyetracker"]
                else None,
                "eye_openness_data": eyetracker.openness
                if eye_settings["use_eyetracker"]
                else None,
                "user_position_data": eyetracker.user_pos
                if eye_settings["use_eyetracker"]
                else None,
            },
            ...
            try:
                for task in task_types:
                    ...
                    for topic in topics_pool:
                        ...
                        print(
                            f"  Generated story pool for {task} including topic {topic}: {pool}"
                        )
                    ...
                    if verbose:
                        ...
                        print(
                            "Sample size:", app_settings[task.replace("-", "_").lower()]
                        )
                    ...
                    # print("---------")
                    story_order.append(
                        [f"/{task.replace('-', '_').lower()}/story_" + i for i in samp]
                    )
                ...
                return sum(
                    story_order, []
                )  # story_order will be a list of lists, this joins everything so that the list is uniform.
...
def write_userdata_to_file(
    ...
    if include_keys != "all":
        keys_to_write = [
            ...
            if (key in include_keys and not key in exclude_keys)
        ]
    else:
    ...
    if data_format == "records":
        with open(filepath, "a") as f:
            for key in keys_to_write:
                f.write(
                    f"{str(key)}: {str(data[key])}{end_line if (keys_to_write.index(key) != len(keys_to_write) - 1) else ''}"
                )
            f.close()
...
def initialise_device(d, **kwargs):
    if d == "eyetracker":
        ...
        device = EyeTracker(
            manager_install_path=kwargs.get(
                "manager_install_path", eye_settings["manager_install_path"]
            )
        )
        try:
            device.connect_eyetracker(
                kwargs.get("eyetracker_index", eye_settings["eyetracker_index"])
            )
        except Exception as error:
        ...
        try:
            device = HRMonitorThread(
                emulate_hr=emulate_hr,
                as_daemon=as_daemon,
                verbose=verbose,
                timestamp_timezone=tz,
            )  # Declare thread wrapper and start thread
        except Exception as error:
            print(
...
def replace_all(text, word_bank, replace_from=None, replace_with=None):
    ...
    try:
        ...
        if any(
            word in [re.sub(r"[^\w\s\d]", "", x).lower() for x in split_text]
...
def set_session_params(data=None, op="update", exclude=[], verbose=False):
    ...
    try:
        print(
            f"\n{'Updating' if op == 'update' else 'Allocating/Resetting'} session parameters{':' if op == 'update' else ''}",
            end="",
        )
...
def create_data_table(
    server_settings, table_name, json_path="bin/db_cols.json", exclude_keys=[]
):
    """
...
def log_template_renders(sender, template, context, **extra):
    ...
    **extra : DICT
    ...
    print(
        f"[SIGNAL.TEMPLATE_RENDERED] Rendered template {template.name or 'string template'} at timestamp {timestamp} in url {page}"
    )
...
def sign_pdf(
    ...
    signer = PDFSigner(
        output_dir=os.path.abspath(f"{os.getcwd()}/data/{idno}"),
    ...
    try:
        ...
        if include_timestamp:
            signer.writeonpdf(
                document,
                datetime.now(signer.ts_region).strftime(signer.ts_format),
                x=230,
                y=730,
...
server = without_keys(
    parse_ini(section="postgresql", eval_datatype=True), {}
)  # Parse server credentials from ini.
app_settings = without_keys(
    parse_ini(section="app_settings", eval_datatype=True), {}
)  # Parse app settings from ini.
eye_settings = without_keys(
    parse_ini(section="eye_tracker", eval_datatype=True), {}
)  # Parse app settings from ini.
hr_settings = without_keys(
    parse_ini(section="hr_tracker", eval_datatype=True), {}
)  # Parse app settings from ini.
...
app_settings["observed_urls"] = tuple(rf"{i}" for i in app_settings["observed_urls"])
...
if hr_settings["use_hrtracker"] and not hr_settings["use_external_app"]:
    ...
    try:
        hr_monitor = initialise_device(
            "hrtracker",
            emulate_hr=bool(hr_settings["emulate_device"]),
            as_daemon=bool(hr_settings["run_thread_as_daemon"]),
            verbose=bool(hr_settings["verbose"]),
            timezone=app_settings["timestamp_timezone"],
        )
        if hr_settings["test_on_startup"]:
            ...
            if datalen == 0:
                raise Exception(
                    "HeartRateMonitorError: Test returned empty dataset. Check device connection!"
                )
            else:
                del datalen
    except Exception as error:
...
if (not exists(app_settings["data_table"])) and app_settings["auto_create_table"]:
    if not create_data_table(
        server, app_settings["data_table"], exclude_keys=app_settings["exclude_columns"]
    ):
        sys.exit(
...
story_relations = read_topic_table(
    file="Human DM Topics.xlsx"
)  # Import relationship between topics, task types, and stories from Excel file.
dir_map = read_dir_tree(
    rootdir="stories/task_types"
)  # Get a dictionary representation of the /stories/task_types directory
...
task_types = [
    x
    for x in list(story_relations[sample(list(story_relations.keys()), 1)[-1]].keys())
    if x not in ["topic_id"]
]  # Get a list of all the task types

# 7. Scan story and questions settings
num_qs_in_story = app_settings[
    "questions_per_story"
]  # No. of questions selected per story.
total_number_of_stories = 0  # How many stories currently exist (calculated from contents of 'stories/task_types').
for x in dir_map:
    total_number_of_stories = total_number_of_stories + len(dir_map[x])
min_topics = app_settings[
    "minimum_topics"
]  # The least amount of topics the user will be allowed to choose.
...
def before_request():
    ...
    timestamp = datetime.now(timezone)
    referrer = (
        request.referrer.split(":".join([host_ip, str(host_port)]))[-1]
        if not request.referrer is None
        else "None"
    )  # The url path from the previous page
    retrival = (
        request.headers["Accept"].split(",")[0] if request.method == "GET" else None
    )  # The content (either html or css) that was downloaded, typically 'text/css' or 'text/html'

    if request.path.startswith(
        app_settings.get("observed_urls", r"/trial")
    ) or referrer.startswith(app_settings.get("observed_urls", r"/trial")):
        if request.method == "POST":
            session["trial_end"] = (timestamp, request.path)
            print(
                f"[BEFORE, POST] Logged 'trial_end'\n Start: {str(session['trial_start'])}\n End: {str(session['trial_end'])}\n Elapsed: {str(session['trial_end'][0] - session['trial_start'][0])}"
            )
            print(
                f"[BEFORE, POST] Timestamp: {timestamp.strftime('%a %b %d %H:%M:%S.%f %Y %Z')} on path {request.path}, referred from {referrer}"
            )
            if app_settings["data_upload"] and not request.path.startswith(r"/trial/"):
                ...
                write_trial_to_db(
                    ...
                    record_path=bool(app_settings.get("record_path", False)),
            ...
            print(
                f"[BEFORE, GET] Logged 'trial_end'\n Start: {str(session['trial_start'])}\n End: {str(session['trial_end'])}\n Elapsed: {str(session['trial_end'][0] - session['trial_start'][0])}"
            )
            print(
                f"[BEFORE, GET] Timestamp: {timestamp.strftime('%a %b %d %H:%M:%S.%f %Y %Z')} on path {request.path}, referred from {referrer}, retrieved {retrival}"
            )
            if app_settings["data_upload"] and not request.path.startswith(r"/trial/"):
    ...
    if request.method == "POST":
        if "existing_participant" in request.form:
            ...
            if not subjectidnumber.isnumeric() and not len(subjectidnumber) == 5:
                ...
                flash(
                    "Invalid: Participant ID must be five a 5 digit number.", "danger"
                )
            ...
            if subjectidnumber not in os.listdir("data"):
                ...
                try:
                    ...
                    flash(
                        f"Invalid: An error occured when loading data for user {subjectidnumber}. Please try a different user ID or select 'I'm a new participant'.",
                        "danger",
                    )
                    return redirect(url_for("login"))
                else:
                    ...
                    demographics = parse_demdata(demographics, subjectidnumber)[
                        -1
                    ]  # Parse data and save it locally
            else:
            ...
            # set_session_params()
            if not set_session_params(
                data=list(data_cols.keys()),
                op="set/reset",
                verbose=bool(app_settings.get("verbose", 0)),
            ):
                ...
                # the app).
                story_order = get_story_order(
                    subjectidnumber,
                    ignore_legacy_story_data=app_settings["ignore_legacy_story_data"],
                    validate_only=True,
                    to_validate=demographics["story_order"],
                )

                # Begin saving data to session
                demographics.update(
                    {"subjectidnumber": subjectidnumber, "trial_index": 0} | story_order
                )  # Joins the two dictionaries, updating the first with the contents of the second
    ...
    if request.method == "POST":
        ...
        for key, value in data.items():
            ...
            if key == "lname" and value == "":
                ...
                data['lname'] = " "
            elif not value == "" and not bool(re.search(pattern_names, value)):
                # If the string does not match this pattern for names, check if
                # it's initials...
                if not bool(re.search(pattern_initials, value)):
                    print(
                        f"Detected non-compliant string in key '{key}' with value '{value}'"
                    )
                    flash(
                        "Name fields must have one or more words containing only letters, separated by either a space or a hyphen (-). Initials fields must be only capital letters optionally separated by a period (.).",
                        "danger",
                    )
        ...
        if not os.path.exists(
            os.path.abspath(
                rf"{os.getcwd()}/data/{session['subjectidnumber']}/informed_consent_signed_{session['subjectidnumber']}.pdf"
            )
        ):
            sign_pdf(
                session.get("subjectidnumber"),
                f"{data['fname']} {data['lname']}",
                data["initials"],
                proxysignature=data["proxyinit"]
                if not data["proxyinit"] == "" or None
                else None,
                stamp_document=True,
                include_timestamp=True,
            )
        ...
        if "continue" in request.form:
            ...
            return send_from_directory(
                path,
                f"informed_consent_signed_{session['subjectidnumber']}.pdf",
                as_attachment=True,
            )
...
def how_feel_pls():
    ...
    if request.method == "POST":
        ...
        if num_stories is None:
            ...
            # user's story order.
            story_indices = get_starting_story_indx(
        ...
        set_session_params(
            data=feeling, op="update", verbose=bool(app_settings.get("verbose", 0))
        )
        ...
        if (
            eye_settings["use_eyetracker"] or hr_settings["use_hrtracker"]
        ) and app_settings["academic_version"] == 1:
...
def setup_biometrics():
    ...
    if request.method == "POST":
        ...
        new_eye_settings = without_keys(
            parse_ini(section="eye_tracker", eval_datatype=True), {}
        )  # Parse app settings from ini.
        new_hr_settings = without_keys(
            parse_ini(section="hr_tracker", eval_datatype=True), {}
        )  # Parse app settings from ini.
        ...
        if EYE_TRACKER_STATUS:
            ...
            flash(
                "Remember to close the eye tracker software before continuing!", "info"
            )
...
def new_participant():
    ...
    if request.method == "POST":
        ...
        # Update session
        set_session_params(
            data=args, op="update", verbose=bool(app_settings.get("verbose", 0))
        )
...
def choose_stories():
    ...
    if request.method == "POST":
        ...
        not_pref_topics = [
            story_blurbs[int(k)]
            for k in range(1, len(story_blurbs) + 1)
            if story_blurbs[int(k)] not in pref_topics
        ]
        ...
        if len(pref_topics) < min_topics:
            return render_template(
                "choose_stories_try_again.html",
                story_blurbs=story_blurbs,
                num_blurbs=len(story_blurbs),
                prev_yes=[int(k) for (k, v) in prefs.items() if v],
        ...
        if story_order is not None:
            ...
            print(
                f"Could not generate story order for user {session['subjectidnumber']}. Please see above for error."
            )
            return redirect(url_for("choose_stories"))
        ...
        if STO_CH:
            ...
            replace_demdata(
                subjectidnumber,
                {"story_order": story_order, "pref_stories": pref_topics},
            )
        else:
            # Update session
            set_session_params(
...
def story_num_refresh():
    ...
    return render_template(
        "story_num.html", number=current_story_indx + 1, content=blurb
    )
...
def context():
    ...
    if task_type == "social":
        if (
            ...
            set_session_params(
                data={"relationship_level": relationship_level},
                op="update",
                verbose=bool(app_settings.get("verbose", 0)),
            )
    ...
    return render_template(
        "context.html",
        content=txt,
        next_prefs=("cost" if task_type == "cost_cost" else "reward"),
    )
...
def rank_prefs(cost_or_reward):
    ...
    if request.method == "POST":
        ...
        if len(vals) != len(set(vals)):
            ...
            print(
                f"Warning: User entered repeated values:\n{vals}\nRendering template '{cost_or_reward}_try_again.html'...\n"
            )
            try_again = f"{cost_or_reward}_try_again.html"
            return render_template(
                try_again, len=len(options), opt_dict=opt_dict, vals=vals
            )

        if cost_or_reward == "cost":
            set_session_params(
                data={"cost_prefs": data},
                op="update",
                verbose=bool(app_settings.get("verbose", 0)),
            )
        else:
            set_session_params(
                data={"reward_prefs": data},
                op="update",
                verbose=bool(app_settings.get("verbose", 0)),
            )
        ...
        return (
            redirect("/prefs/cost")
            if (cost_or_reward == "reward" and task_type != "benefit_benefit")
...
def context_refresh():
    ...
    if task_type == "social":
        if (
            ...
            txt, _ = replace_all(
                txt, app_settings["relation_levels"], replace_with=relationship_level
            )
    set_session_params(
        data={"relevant_questions": choose_questions(session)},
        op="update",
        verbose=bool(app_settings.get("verbose", 0)),
    )
...
def rank_prefs_again(cost_or_reward):
    ...
    if request.method == "POST":
        ...
        return (
            redirect("/refresh_prefs/cost")
            if (cost_or_reward == "reward" and task_type != "benefit_benefit")
            else redirect("/trial_end")
        )
...
def trial_html(loc_trial_num):
    ...
    if request.method == "POST":
        ...
        if next_trial < num_qs_in_story:
            ...
            set_session_params(
                data={
                    "current_story_indx": current_story_indx + 1,
    ...
    if task_type in ["multi_choice", "benefit_benefit", "cost_cost"]:
        ...
        q = Markup(q)
        return render_template(
            "trial_n_y.html", trial_number=trial_index + 1, question=q
        )
...
def get_story_relevance():
    ...
    if request.method == "POST":
        ...
        return (
            redirect("/story_num_overall")
            if next_story_index <= max_story_indx
            else redirect("/total_end")
        )
...
def total_end():
    ...
    if app_settings["data_upload"]:
        write_trial_to_db(
            (0, 0) if not task_type in ["multi_choice"] else (0, 0, 0, 0),
            record_path=bool(app_settings.get("record_path", False)),
            exclude_keys=session["exclude"],
        )
    ...
    set_session_params(
        ...
        op="update",
        verbose=bool(app_settings.get("verbose", 0)),
    )  # Signal that the app parameters need to be reset.

    if request.method == "POST":
        data = request.form.to_dict()
        set_session_params(
            data={"session_notes": data},
            op="update",
            verbose=bool(app_settings.get("verbose", 0)),
        )
        if app_settings["data_upload"]:
    ...
    return render_template(
        "total_end.html",
        app_version="academic"
        if app_settings.get("academic_version", 0) == 1
        else "online",
    )
...
def final_end():
    ...
    return render_template(
        ...
        completed_stories=session.get("current_story_indx", 0),
        total_stories=len(session.get("story_order", [])),
    )
...
if __name__ == "__main__":
    ...
    if args.mode == "prod":
        ...
        print(
            f"Forced 'academic_version' from {app_settings['academic_version']} to 0."
        )
        app_settings.update({"academic_version": 0, "verbose": int(args.verbose)})

        print("Waitress")
        serve(
            app, host=host_ip, port=host_port, threads=threads, url_prefix="/humans-app"
        )
