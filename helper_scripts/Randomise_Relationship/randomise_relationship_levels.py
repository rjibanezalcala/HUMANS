...
# Replace the relationship word with one from the word bank
    ...
    if any(word in [ re.sub(r'[^\w\s\d]', '', x).lower() for x in split_text] for word in word_bank):
        if replace_from is None:
            matches = None
            results = []
            # results = [ re.search(r'\b'+re.sub(r'[^\w\s\d]', '', word)+r'\b', text, re.IGNORECASE).group(0) for word in word_bank if not (re.search(r'\b'+re.sub(r'[^\w\s\d]', '', word)+r'\b', text, re.IGNORECASE) is None) ]
...
if __name__ == "__main__":
    ...
    #story_num = f"story_{str(story)}"
    story_num = "story_21"
    text = read_file(root_dir, story_num, 'questions')
    print(f"\n\n-------------- STORY {story_num} -------------------")
    i = 0
    for key, value in text.items():
        ...
        print(f"\n QUESTION #{str(i)}: {str(key)}")
        print(">> Run replace_all with randomly picked relationship level:")
        print(f'>>> Text before replacement:\n"{text}"')
